<template>
  <div id="allFormation">
      ...
      <div class="main-title">
        ...
        <div class="subselect" v-if="is_domaineLoaded && domaines">
          <a
            ...
            :key="dom.id"
            @click="LoadThemesWithFormations(dom.id)"
          >
            {{ dom.name }}
          </a>
        </div>
    ...
    <div class="container-fluid pb-5">
      ...
      <ul
        v-if="
          themes_by_domaine && themes_by_domaine.length > 0 && is_themeLoaded
        "
        class="onglet w-100 nav nav-tabs"
        id="myTab"
        role="tablist"
      >
        <!-- button-right -->
        ...
        <li
          class="w-auto"
          v-for="(thm, thmIdx) in themes_by_domaine"
          :key="thmIdx"
          @click="handleAction('formationStore/setFormationsByTheme', thm.id)"
        >
          <router-link
            :class="(currThemeId == thm.id && 'nav-link active') || 'nav-link'"
            :id="`theme${thm.id}-tab`"
            :to="`#theme${thm.id}`"
            data-toggle="tab"
            role="tab"
            :aria-controls="`theme${thm.id}`"
            aria-selected="true"
          >
      ...
      <ul
        v-if="
          is_themeLoaded && themes_by_domaine && themes_by_domaine.length === 0
        "
      ...
      <!-- LOADING .. -->
      <div
        v-else-if="!has_formationError && !is_formationLoaded"
        class="loading"
      >
...
export default {
  ...
  mounted() {
    this.domaine_param = this.$route.params.domaine_param
      ? Math.round(this.$route.params.domaine_param)
      : undefined;
    this.theme_param = this.$route.params.theme_param
      ? Math.round(this.$route.params.theme_param)
      : undefined;
  },
  async created() {
    ...
    await this.$store.dispatch(
      "formationStore/setCurrDomaineId",
      this.domaine_param
    );
    await this.$store.dispatch(
      "formationStore/setCurrThemeId",
      this.theme_param
    );
    ...
    if (this.domaine_param) {
      await this.$store.dispatch(
        "formationStore/setDomaineById",
        this.domaine_param
      );
      await this.$store.dispatch(
        "formationStore/setThemesByDomaine",
        this.domaine_param
      );
    } else {
      // by defauls
