<template>
  <section class="container-fluid py-5">
    <div class="mysys-search-box d-flex flex-nowrap">
      <button class="mysys-search-btn bg-primary">
        <i class="fa fa-search"></i>
      </button>
      <input
        class="mysys-search-input"
        type="search"
        name="search-input"
        id="searchInput"
        placeholder="Que cherchez-vous"
        v-model="search_value"
        @change="Search(search_value)"
    ...
    <div class="mysys-search-result d-none" id="searchResult">
      <ul v-if="formations" class="mysys-search-list w-100 list-unstyled">
        <li
          v-for="(forma, formaIdx) in formations"
          :key="formaIdx"
          class="mysys-search-item text-secondary"
        >
          <i class="material-icons align-middle pr-4">search</i>
          <router-link
            :to="{ name: 'detailformation', params: { form_param: forma.id } }"
            >{{ forma.name }}</router-link
          >
        </li>
      </ul>
